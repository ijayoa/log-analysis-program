...
def most_popular_authors():
    """Return the most popular authors sort by descending order highest total article views."""
    db = psycopg2.connect("dbname=news")  # open database connection
    cursor = db.cursor()
    try:
...
def days_with_errors():
    db = psycopg2.connect("dbname=news") # connect to database
    cursor = db.cursor() # open cursor to scan records
    try:
    		# query : use log_report view to calculate percent and filter those greater than 1
        cursor.execute("select day, round(error * 100.0 / requests, 1) as percent from log_report group by day,error,requests having round(error * 100.0 / requests, 1) > 1 order by percent desc;")
        rows = cursor.fetchall() # fetch results from database
        # loop through to display retrieved records
        for i in rows:
            print('{} - {}% errors'.format(i[0], i[1]))  # output string format
    finally:
        db.close() # close database connection
